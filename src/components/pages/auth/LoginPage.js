...
const LoginPage = ({ user:{errors, success, loggedin}, theme:{light} , login, clearErrors }) => {
  //iniatialise state
  ...
  return (
    <div className={`wrap-auth ${light ? 'light-theme' :'dark-theme'}`}>
      <form onSubmit={onSubmit} className="container auth-form" autoComplete="" style={{boxShadow: !light && '0px 3px 10px 0px rgb(241 234 234 / 35%)'}}>
        <h4 className="mb-4 text-center s12 auth-form-title">Login</h4>
        <div className="row">
        ...
        {success && (
          <div
            className="text-center italic"
            style={{ color: "#039be5", fontSize:"16px" }}
          >
            {success}
          </div>
        )}
        ...
        <div className="w-full text-right mt-4">
          <button type="submit" className="waves-effect waves-light btn" disabled={errorMessage||errors||success}>
            <i className="material-icons left">save</i>Login
          </button>
        </div>
      </form>
      <br />
      <p className="text-center">
        You don't have an account?, Please <Link to="/register">Register</Link><br />
        Forgot your password?, Please <Link to="/resetpassword">Reset password</Link><br />
      </p>
    </div>
  );
};
